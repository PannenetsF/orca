...
import { buildElectronRunAsNodeEnv } from './cli-launch-redirect'
 ...
 *
 * The CLI launch redirect owns $APPIMAGE/$APPDIR launches; this covers the
 ...
 * ($APPIMAGE/$APPDIR — owned by the CLI launch redirect), and `serve` (a real
...
export function getExtractedLinuxCliArgs(
  ...
  // Why: node mode already runs the CLI directly; the CLI launch redirect handles
  ...
  const firstPositionalIndex = findCliCommandIndex(
    cliArgs,
    options.commandNames.map((name) => [name]),
    CLI_GLOBAL_VALUE_FLAGS
  )
  // Why: `serve` opens real browser surfaces and owns its own Electron launch, so
  // it must NOT be rerouted into node mode here.
  if (firstPositionalIndex === -1 || cliArgs[firstPositionalIndex] === 'serve') {
    return null
  }
  return cliArgs
}
